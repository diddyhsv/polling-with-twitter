...
def calcResult(name, pop, blat, bbla, bwhi, tlat, tbla, twhi, percl, percb, percw):
    ...
    if(((len(set(tlat)) > 0) & (len(set(blat)) > 0))):
        for user in blat:
            if(user in tlat):
        ...
        print("No Men Tweets Registered in This District")
    print(f"Women ({percb*100}%)")
    if(((len(set(tbla)) > 0) & (len(set(bbla)) > 0))):
        for user in bbla:
            if(user in tbla):
        ...
        print("No Women Tweets Registered in This District")
    print(f"Unknowns ({percw*100}%)")
    if(((len(set(twhi)) > 0) & (len(set(bwhi)) > 0))):
        for user in bwhi:
            if(user in twhi):
        ...
        print("No Unknown Tweets Registered in This District")
...
with open('/Volumes/TOSHIBA/usawithgendercongress.txt', 'rb') as f:
...
with open("/Volumes/TOSHIBA/usawithgendercongress.txt", 'wb') as f:
