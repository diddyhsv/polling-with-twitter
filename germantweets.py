...
fName = '/Volumes/TOSHIBA/tweetsgermany201031.txt' # We'll store the tweets in a text file.
sin = "2020-10-31"
til = "2020-11-01"
...
with open(fName, 'w') as f:
    while tweetCount < maxTweets:
        try:
            if (max_id <= 0):
                if (not sinceId):
                    new_tweets = api.search(q=searchQuery, count=tweetsPerQry, lang="de", tweet_mode='extended', since=sin, until=til)
                    time.sleep(5)
                    ...
                    new_tweets = api.search(q=searchQuery, count=tweetsPerQry,
                                            since_id=sinceId, lang="de", tweet_mode='extended', since=sin, until=til)
                    for elem in new_tweets:
                          blob = TextBlob(elem.full_text)
                          totalpolarity = totalpolarity + blob.sentiment.polarity
                          try:
                ...
                if (not sinceId):
                    time.sleep(5)
                    new_tweets = api.search(q=searchQuery, count=tweetsPerQry,
                                            max_id=str(max_id - 1),lang="de",tweet_mode='extended', since=sin, until=til)
                    for elem in new_tweets:
                          blob = TextBlob(elem.full_text)
                          totalpolarity = totalpolarity + blob.sentiment.polarity
                          try:
